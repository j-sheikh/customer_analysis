...
Because the values in the different date columns has mostly the same form(yyyy-mm-dd),i specified a regex for this form.I can change the regex later for different dates,if a new dataset requires this.
"""
...
Besides that, some colors has double entries (brown/brwon,oliv/olive and blue/blau).
...
"""The age has a normal distribution.We can see that the age range is between 18-80 years old.It's unlikley to have a customer younger than 18 (age restriction) and over 80 years old.Therefore i remove all observations less than 18 and greater than 80"""
...
Deliveries are between Monday and Saturday.The most deliveries are on a Tuesday. Monday and Wednesday are almost equal.
It get's less at the weekend.With zero deliveries at Sunday which is obvious,because delivery companies are only working from Monday to Saturday.
"""
...
"""Here we can see the delivery time in days. We have a lot of outliers, with a max delivery duration of 175.These are obviously outliers,we need to remove."""
